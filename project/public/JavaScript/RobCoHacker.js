    ...
    if ($(".selected").length <= 0) {
        ...
        if (currentArea == "Menu") {
            if (keys['UP'] || keys['LEFT']) {
                ...
                if (jQuery.inArray(current, mainMenu) || current == "NoviceDiff") {
                    $(".modeSelection").remove();

                    selectedDiff = current;

                    let indi1 = randInt(16);
                    let indi2 = randInt(16);
                    let indi3 = randInt(16);
                    ...
                    for (x = 1; x <= 5; x++) {
                        $(".playSpace").append('<div class="column col' + x + '">');
                        if(x < 5){
                            for (y = 1; y <= 17; y++) {
                                $(".col" + x).append('<div class="row row' + y + '"></div>');
                            }
                        }else{
                            $(".col5").append("<div class='terminalMessages'></div>");
                            $(".col5").append("<div class='terminalSelected'></div>");
                            $(".terminalSelected").append("<p></p>");
                        }
                    }
                        ...
                        for (let y = 0; y < 17; y++) {
                            ...
                            if (x % 2 == 0) {
                                for (let z = 3; z >= 0; z--) {
                                    ...
                                    if (z == 3) {
                                        let incrementTemp = Math.floor(indicatorArr[3] / 16);
                                        indicatorArr[3] += 12;
                                        if (Math.floor(indicatorArr[3] / 16) > incrementTemp) {
                                            indicatorArr[2] += 1;
                                            if (indicatorArr[2] % 16 == 0) {
                                                indicatorArr[1] += 1;
                                                if (indicatorArr[1] % 16 == 0) {
                                                    indicatorArr[0] += 1;
                                                }
                                            }
                                        }
                                ...
                                string = "0x" + string.split("").reverse().join("");
                                $(".row" + (y + 1)).within(".col" + (x + 1)).html("<p>" + string + "</p>");

                            } else {
                                $(".row" + (y + 1)).within(".col" + (x + 1)).append("<p></p>");
                                for (let z = 0; z < 12; z++) {

                                    let charRandomizer = randInt(charRandVal - (diffWordcount[selectedDiff] * 2))
                                    if(charRandomizer > 1 & loadWord.length <= 0 || forceSpace || wordsUsed == diffWordcount[selectedDiff] || (x == 3 & y == 16 & loadWord.length == 0)){
                                        charRandVal--;
                                        if(forceSpace){
                                            forceSpace = false;
                                        }
                                        let randomChar = fillerCharacters[randInt(fillerCharacters.length) - 1]
                                        
                                        
                                        if (randomChar == "(" || randomChar == "{" || randomChar == "[" || randomChar == "<") {
                                            string += "<span class='char" + (z + 1) + " open'>" + randomChar + "</span>";
                                            
                                        } else if (randomChar == ")" || randomChar == "}" || randomChar == "]" || randomChar == ">") {
                                            string += "<span class='char" + (z + 1) + " close'>" + randomChar + "</span>";
                                            
                                        } else {
                                            string += "<span class='char" + (z + 1) + "'>" + randomChar + "</span>";
                                        }
                                    }else if(charRandomizer == 1 & loadWord.length <= 0){

                                        charRandVal = 80;
                                        while(loadWord.length < 3){
                                            loadWord = words[randInt(words.length) - 1];
                                            wordsUsedList.push(loadWord);
                                        }

                                        string += "<span class='char" + (z + 1) + " word word" + (wordsUsed + 1) + "'>" + loadWord[0] + "</span>";
                                        loadWord = loadWord.substring(1, loadWord.length);
                                        if(loadWord.length <= 0){
                                            wordsUsed++;
                                            forceSpace = true;
                                        }
                                    }else if(loadWord.length > 0){
                                        string += "<span class='char" + (z + 1) + " word word" + (wordsUsed + 1) + "'>" + loadWord[0] + "</span>";
                                        loadWord = loadWord.substring(1, loadWord.length);
                                ...
                                $("p").within(".row" + (y + 1)).within(".col" + (x + 1)).append(string);
                            }
                        }
                    }
                    correctWord = wordsUsedList[randInt(wordsUsed) - 1];
                    currentArea = "Playing";
                }
            ...
            // currently skips multiple places
            $.each(keys, function (key, value) {

                let currentSelect = $(".selected").attr("class");
                currentSelect = currentSelect.replace(" selected", "");
                currentSelect = currentSelect.replace("char", "");

                let row = $(".selected").closest("div[class^='row']").attr("class");
                row = row.replace("row ", "");
                row = row.replace("row", "");

                let column = $(".selected").closest("div[class^='column']").attr("class");
                column = column.replace("column ", "");
                column = column.replace("col", "");

                column = parseInt(column);
                row = parseInt(row);
                current = parseInt(currentSelect);
                ...
                if (key == "UP") {
                    ...
                    if (row != 1) {
                        ...
                        $("span.char" + current).within(".row" + (row - 1)).within(".col" + column).addClass("selected");
                    ...
                    if (current != 1) {
                        $(".selected").removeClass("selected");
                        $(".selectedSpan").removeClass("selectedSpan");
                        $("span.char" + (current - 1)).within(".row" + row).within(".col" + column).addClass("selected");
                    } else if (column != 2) {
                        $(".selected").removeClass("selected");
                        $(".selectedSpan").removeClass("selectedSpan");
                        $("span.char12").within(".row" + row).within(".col2").addClass("selected");
                    }
                } else if (key == "RIGHT") {
                    if (current != 12) {
                        $(".selected").removeClass("selected");
                        ...
                        $(".selected").removeClass("selected");
                        $(".selectedSpan").removeClass("selectedSpan");
                        $("span.char1").within(".row" + row).within(".col4").addClass("selected");
                    }

                } else if (key == "ENTER" & !keys["UP"] & !keys["DOWN"] & !keys["LEFT"] & !keys["RIGHT"]) {
                    if (($(".selected").hasClass("open") || $(".selected").hasClass("close") || $(".selected").hasClass("word")) & $(".selectedSpan").length > 0 & !$(".selected").hasClass("activated")) {
                        if ($(".selected").hasClass("word")) {
                            let like = 0;
                            let tempCorrect = correctWord;
                            let tempSelected = $(".selectedSpan").text();
                            $(".terminalMessages").append($(".terminalSelected").html());

                            if(tempCorrect != tempSelected){
                                for(x = 0; x < tempSelected.length; x++){
                                    for(y = 0; y < tempCorrect.length; y++){
                                        if(tempCorrect[y] == tempSelected[x]){
                                            like++;
                                            tempCorrect = replaceAt(tempCorrect, y, "");
                                            break;
                                        }
                                    }
                                }
                                $(".terminalMessages").append("<p>\>Likeness=" + like + "</p>");
                ...
                if(currentArea = "Playing"){
                    ...
                    row = $(".selected").closest("div[class^='row']").attr("class");
                    row = row.replace("row ", "");
                    row = row.replace("row", "");

                    column = $(".selected").closest("div[class^='column']").attr("class");
                    column = column.replace("column ", "");
                    column = column.replace("col", "");
                    ...
                    if ($(".selected").hasClass("open")) {
                        let rowText = $(".selected").parent("p").text();
                        rowText = rowText.substr(current);

                        let selectedChar = $(".selected").text();
                        let closeChar;

                        for (x = 0; x < dudCharas.length; x += 2) {
                            if (dudCharas[x] == selectedChar) {
                                closeChar = dudCharas[x + 1];

                            }
                        }
                        for (x = 0; x < rowText.length; x++) {
                            if (rowText[x] == selectedChar) {
                                break;
                            }
                            if(!checkDudCharas(rowText[x])){
                                break;
                            }
                            if (rowText[x] == closeChar) {
                                let matchChar = current + x + 1;
                                for (y = current; y <= matchChar; y++) {
                                    $(".char" + y).within(".row" + row).within(".col" + column).addClass("selectedSpan");
                                }
                                break;
                            }
                        }
                        ...
                        let rowText = $(".selected").parent("p").text();
                        rowText = rowText.substr(0, current - 1);

                        let selectedChar = $(".selected").text();
                        let openChar;

                        for (x = 1; x < dudCharas.length; x += 2) {
                            if (dudCharas[x] == selectedChar) {
                                openChar = dudCharas[x - 1];

                            }
                        }
                        for (x = rowText.length - 1; x >= 0; x--) {
                            if (rowText[x] == selectedChar) {
                                break;
                            }
                            if(!checkDudCharas(rowText[x])){
                        ...
                        let wordSelect = $(".selected").attr("class");
                        wordSelect = wordSelect.replace(" selected", "");
                        wordSelect = wordSelect.replace("char" + current, "");
                        wordSelect = wordSelect.replace("word word", "");

                        currentWord = parseInt(wordSelect);

                        $(".word" + currentWord).addClass("selectedSpan");
                    }
                }
...
function checkDudCharas(chara){
    for(y = 0; y < fillerCharacters.length; y++){
        if(fillerCharacters[y] == chara){
            return true;
        }
    }
    return false;
